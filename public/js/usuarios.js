...
function mostrarUsuarios(listaUsuarios) {
    ...
    tbody.innerHTML = listaUsuarios.map(user => `
        <tr>
            <td><strong>${user.nombre} ${user.apellido}</strong></td>
            <td>${user.email}</td>
            <td>${user.telefono || '-'}</td>
            <td>
                <span class="badge ${user.id_rol === 1 ? 'badge-admin' : 'badge-cliente'}">
                    <i class="fas ${user.id_rol === 1 ? 'fa-user-shield' : 'fa-user'}"></i>
                    ${user.rol}
                </span>
            </td>
            ...
            <td>
                <div class="action-buttons">
                    <button class="btn-icon btn-view" onclick="verDetalles(${user.id_usuario})" title="Ver detalles">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn-icon btn-edit" onclick="editarUsuario(${user.id_usuario})" title="Editar">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn-icon btn-delete" onclick="confirmarEliminar(${user.id_usuario}, '${user.nombre} ${user.apellido}')" title="Eliminar">
...
function filtrarUsuarios() {
    ...
    if (filterRol) {
        usuariosFiltrados = usuariosFiltrados.filter(user => 
            user.id_rol === parseInt(filterRol)
        );
    }
    ...
    if (filterAdmin) {
        usuariosFiltrados = usuariosFiltrados.filter(user => 
            user.id_administrador === parseInt(filterAdmin)
        );
    }
...
async function editarUsuario(id) {
    try {
        ...
        if (usuarioEditar.id_rol === 2 && usuarioEditar.id_administrador) {
            ...
            document.getElementById('id_administrador').value = usuarioEditar.id_administrador;
        }
...
async function guardarUsuario(e) {
    ...
    try {
        ...
        if (formData.id_rol === 2) {
            formData.id_administrador = parseInt(document.getElementById('id_administrador').value);
        }
    ...
    try {
        ...
        const detallesHTML = `
            ...
            <div class="detalle-item">
                ...
                <div class="detalle-value">${formatDate(user.fecha_registro)}</div>
            </div>
        `;
