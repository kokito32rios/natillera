...
async function cargarEstadisticas() {
    try {
        ...
        const totalAhorrado = ahorros.reduce((sum, a) => sum + parseFloat(a.saldo_actual || 0), 0);
        document.getElementById('totalAhorrado').textContent = formatCurrency(totalAhorrado);
        
        const responsePrestamos = await fetch('/api/prestamos');
        const prestamos = await responsePrestamos.json();
        const prestamosActivos = prestamos.filter(p => p.estado === 'activo' || p.estado === 'moroso');
...
async function cargarAportesRecientes() {
    try {
        ...
        container.innerHTML = aportesRecientes.map(aporte => `
...
async function cargarActividadesProximas() {
    try {
        ...
        const actividadesProximas = actividades
            .filter(a => a.estado === 'planeada' && new Date(a.fecha) >= hoy)
        ...
        container.innerHTML = actividadesProximas.map(actividad => `
            <div class="activity-item">
